(function(ThemeBuilder) {
    ...
    LessFontPlugin.prototype = {
        process: function(css, extra) {
            ...
            css = css.replace(/url\(fonts\/Roboto/gi, "url(Content/DevExtreme/css/fonts/Roboto");
            return css;
        }
    };
    ...
    ThemeBuilder.LessTemplateLoader = function() {
        ...
        this.compileScss = function(less, metadata) {
            ...
            var sass = preCompiler.process(less);

            var compiler = new Sass();
            compiler.compile(sass, function(result) {
                if(result.status === 0) {
                    var postCompiler = new LessMetadataPostCompilerPlugin(less, compiledMetadata);
                    postCompiler.process(result.text);

                    //TODO what about fonts?
                    d.resolve(compiledMetadata, result.text);
                } else {
                    parentIframeNotifier.notify("handleError", { message: result.message });
                    d.reject();
                }
        ...
        this.analyzeBootstrapTheme = function(theme, colorScheme, metadata, bootstrapMetadata, customLessContent, version) {
            ...
            $.each(bootstrapMetadata, function(key, value) {
                preLessString += value + ": dx-empty" + (version === 4 ? " !default" : "") + ";";
            });
            ...
            if(version === 3) {
                that.compileLess(preLessString + customLessContent, {}, bootstrapMetadata).done(function(compiledMetadata) {
                    processDxTheme(compiledMetadata);
                });
            } else if(version === 4) {
                var defaultBootstrapVariablesUrl = "https://raw.githubusercontent.com/twbs/bootstrap/v4-dev/scss/_variables.scss";
                var defaultBootstrapFunctionsUrl = "https://raw.githubusercontent.com/twbs/bootstrap/v4-dev/scss/_functions.scss";

                $.when($.get(defaultBootstrapFunctionsUrl), $.get(defaultBootstrapVariablesUrl))
                    .done(function(functions, defaultScss) {
                        that.compileScss(functions[0] + customLessContent + defaultScss[0] + preLessString, bootstrapMetadata).done(function(compiledMetadata) {
                            processDxTheme(compiledMetadata);
                        });
                    })
                    .fail(function() {
                        that.compileScss(customLessContent + preLessString, bootstrapMetadata).done(function(compiledMetadata) {
                            processDxTheme(compiledMetadata);
                        });
                    });
            }
        ...
        this.getCSSFromPage = function(metadataVersion) {
            ...
            return css
                .replace(/url\(Content\/DevExtreme\/css\/icons\/dxicons/gi, "url(icons/dxicons")
                .replace(/url\(Content\/DevExtreme\/css\/fonts\/Roboto/gi, "url(fonts/Roboto");
        };
