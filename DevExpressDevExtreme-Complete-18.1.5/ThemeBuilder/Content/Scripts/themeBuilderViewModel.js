(function(ThemeBuilder) {
    ...
    var ViewModel = function(metadataRepository, historyChangesManager, previewIframeNotifier, cssBeautifier) {
            ...
            if(updatedMetadata) {
                ...
                if(updatedMetadata.version !== metadataVersion) {
                    DevExpress.ui.dialog.confirm(
                        "Your version of metadata does not match the Theme Builder v" + metadataVersion + ".<br />Some Less variables may become unavailable.<br />Do you want to continue?", "Warning: Version Mismatch")
        ...
        this.bootstrapFileUploaded = function(e) {
            var file = e.value[0];
            if(file) {
                var fileReader;
        ...
        this.bootstrapAnalyzePopupOptions = {
            visible: ko.observable(false),
            title: "Analyze Bootstrap Variables",
            height: "auto",
            width: 600,
            dragEnabled: false
        };

        this.bootstrapAnalyzeVersion = ko.observable();

        this.showAnalyzeBootstrapPopup = function() {
            that.bootstrapAnalyzePopupOptions.visible(true);
        };

        this.analyzeBootstrap = function(customLess, version) {
            ...
            previewIframeNotifier.notify($previewIframe, "analyze", {
                ...
                currentTheme: this.currentTheme(),
                currentColorScheme: this.currentColorScheme(),
                bootstrapMetadata: that.bootstrapAnalyzeVersion().version === 3 ? ThemeBuilder.__bootstrap_metadata() : ThemeBuilder.__bootstrap4_metadata(),
                customLessContent: customLess,
                version: that.bootstrapAnalyzeVersion().version
            });
        ...
        this.treeItemClick = function(e) {
            if(e.event && e.event.target && $(e.event.target).closest(".dx-treelist-empty-space").length) {
                return;
            }
